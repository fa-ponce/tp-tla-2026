...
CompilationStatus LoadLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, LOAD);
	_logTokenAction(__FUNCTION__, token);
...
CompilationStatus SaveLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, SAVE);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus FilterLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, FILTER);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus MutateLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, MUTATE);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus SelectLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, SELECT);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus SortLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, SORT);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus ByLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, BY);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus LimitLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, LIMIT);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus AscLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, ASC);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus DescLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, DESC);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus IfLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, IF);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus ThenLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, THEN);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus ElseLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, ELSE);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus ApplyLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, APPLY);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus AndLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, AND);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus OrLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, OR);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus NotLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, NOT);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus LetLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, LET);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus FunctionLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, FUNCTION);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus ReturnLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, RETURN);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus PipelineLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, PIPELINE);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus RequireLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, REQUIRE);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus ForLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, FOR);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus InLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, IN);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus EqualLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, EQUAL);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus NotEqualLexemeAction(){
	Token* token = createToken(_lexicalAnalyzer, NOT_EQUAL);
	_logTokenAction(__FUNCTION__, token);
	CompilationStatus status = pushToken(_lexicalAnalyzer, token);
	destroyToken(token);
	return status;
...
CompilationStatus StringLexemeAction(){
	...
	int length = token->length - 2; // sin las comillas
	char * value = (char *) calloc(length + 1, sizeof(char));
	strncpy(value, token->lexeme + 1, length);
